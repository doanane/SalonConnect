...
def update_user_profile(
    ...
    """Update user profile"""
    return AuthService.update_user_profile(db, current_user.id, profile_data)
...
def get_customer_dashboard(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
def get_vendor_dashboard(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
@router.delete("/me/profile/remove-picture")
def remove_profile_picture(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
