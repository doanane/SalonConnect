...
class PaymentService:
    ...
    def initiate_payment(db: Session, booking_id: int, customer_id: int):
        ...
        payment = Payment(
            ...
            currency="GHS",  # CHANGED: Explicitly set to GHS
        ...
        try:
            ...
            # Paystack expects amount in kobo (100 pesewas = 1 Ghana Cedi)
            # So multiply by 100 to convert Cedis to pesewas
            payload = {
                "email": booking.customer.email,
                "amount": int(booking.total_amount * 100),  # Convert GHS to pesewas
                "reference": payment_reference,
                "callback_url": f"{settings.FRONTEND_URL}/payment/success",
                "currency": "GHS",  # CHANGED: Specify Ghana Cedis
                "metadata": {
                    "booking_id": booking_id,
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    "amount": booking.total_amount,
                    "currency": "GHS"  # CHANGED: Add currency to response
                }
            else:
